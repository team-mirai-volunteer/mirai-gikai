...
export function BillForm({ bill, mode }: BillFormProps) {
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const isEdit = mode === "edit" && bill;
  const schema = isEdit ? billUpdateSchema : billCreateSchema;
  ...
  const form = useForm<BillCreateInput | BillUpdateInput>({
    resolver: zodResolver(schema),
    defaultValues: getDefaultValues(),
  });

  async function onSubmit(data: BillCreateInput | BillUpdateInput) {
    setIsSubmitting(true);
    setError(null);

    try {
      if (isEdit) {
        await updateBill(bill.id, data as BillUpdateInput);
      } else {
        await createBill(data as BillCreateInput);
      }
    } catch (err) {
      setError(
        err instanceof Error
          ? err.message
          : `${isEdit ? "更新" : "作成"}中にエラーが発生しました`
      );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{isEdit ? "議案基本情報編集" : "議案新規作成"}</CardTitle>
      </CardHeader>
      <CardContent>
        <Form {...form}>
