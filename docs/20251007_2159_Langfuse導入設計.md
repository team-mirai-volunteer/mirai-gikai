...
現在のシステムプロンプト（`route.ts`の87-107行目）の1つをLangfuseからリモート取得し、Vercel AI SDKに統合する実装を完了する。
...
- 難易度別プロンプト（`getDifficultyInstructions`）もLangfuse管理に移行
- プロンプトキャッシング機構の追加
